...
As terminal height shrinks, the UI SHALL hide decorative elements before functional ones, in this order: logo first, then slash-menu rows (already 3→2→1), then vertical padding. The prompt input, at least 5 result rows, the now-playing row (when playing), and the status bar SHALL remain visible at any height ≥ 12 rows.
